...
try {
  ...
  await client.query("TRUNCATE stop_times, transfers, calendar_dates, calendars, trips, stops CASCADE");
  await client.query(readFileSync("migrations/0000_gtfs.sql", "utf8"));
  const stops = csv(join(feed, "stops.txt")); const trips = csv(join(feed, "trips.txt")); const times = csv(join(feed, "stop_times.txt")); const transfers = csv(join(feed, "transfers.txt")); const calendars = csv(join(feed, "calendar.txt")); const exceptions = csv(join(feed, "calendar_dates.txt"));
