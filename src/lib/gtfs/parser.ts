...
export function parseStopTimes(source: string): StopTime[] { return rows(source).map((row) => ({ tripId: row.trip_id, stopId: row.stop_id, arrival: gtfsTimeToSeconds(row.arrival_time), departure: gtfsTimeToSeconds(row.departure_time), sequence: Number(row.stop_sequence) })); }
